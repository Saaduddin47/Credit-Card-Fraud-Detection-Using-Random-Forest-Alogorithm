...
# Global variables
global filename, cls, X, Y, X_train, X_test, y_train, y_test
global random_acc, clean, attack, total
...
def runRandomForest():
    headers = ["Time","V1","V2","V3","V4","V5","V6","V7","V8","V9","V10","V11","V12","V13",
...
def graph():
    ...
    plt.bar(y_pos, height, color=['dodgerblue', 'seagreen', 'tomato'])
    plt.xticks(y_pos, bars)
    plt.title("Transaction Classification Summary")
...
title.config(bg='lightyellow', fg='dodger blue')  
...
# Buttons
uploadButton = Button(main, text="Upload Credit Card Dataset", command=upload, **button_style)
uploadButton.place(x=50, y=550)

modelButton = Button(main, text="Generate Train & Test Model", command=generateModel, **button_style)
modelButton.place(x=50, y=600)

runrandomButton = Button(main, text="Run Random Forest Algorithm", command=runRandomForest, **button_style)
runrandomButton.place(x=50, y=650)

predictButton = Button(main, text="Detect Fraud From Test Data", command=predicts, **button_style)
predictButton.place(x=450, y=550)

graphButton = Button(main, text="Clean & Fraud Transaction Detection Graph", command=graph, **button_style)
graphButton.place(x=450, y=600)

exitButton = Button(main, text="Exit", command=exit, **button_style)
